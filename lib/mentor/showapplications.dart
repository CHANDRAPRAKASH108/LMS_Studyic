...
class _FetchApplicationsState extends State<FetchApplications> {
  ...
  Widget build(BuildContext context) {
            ...
            content: Column(
                children: <Widget>[
                    ...
                    child: RaisedButton(
                      onPressed: ()async{
                        await FirebaseFirestore.instance.collection("Mentee").document(email).collection("Applications").document(email)
                            .updateData({"teacher status":"Approved"});
                        await Firestore.instance.collection(hostel).document()
                        .setData({
                          'name': name,
                          'email': email,
                          'regd no': regd_no,
                  ...
                  Container(
                    ...
                    child: RaisedButton(
                      onPressed: ()async{
                        await Firestore.instance.collection("Mentee").document(email).collection("Applications").document(email)
    ...
    return StreamBuilder(
        stream: Firestore.instance.collection("Mentor").document("Application").collection(id).snapshots(),
