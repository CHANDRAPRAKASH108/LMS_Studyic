...
class _StudentMentorState extends State<StudentMentor> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('Mentor Applications'),
...
class _ApplicationState extends State<Application> {
  ...
  Widget build(BuildContext context) {
    ...
    SubmitApplication()async{
      ...
      await FirebaseFirestore.instance.collection("Mentee").document(email).get()
      ...
      if(studentname == null || studentregd ==null || studentroll==null || studentdescription ==null || from == null || till ==null || onselected == null || roomnumber ==null){
        ...
        await FirebaseFirestore.instance.collection("Mentor").document("Application").collection(mentorid).document()
            .setData({
          'name':studentname,
      ...
      child: Container(
        child: Column(
          children: <Widget>[
            ...
            Container(
              child: Text('Select your hostel',style: TextStyle(fontSize: 18.0,fontWeight: FontWeight.bold),),
            ),
            Container(
              ...
              child: DropdownButton(
                value: onselected,
                items: const[
                  DropdownMenuItem(child: Text('OBH'),
                  value: "OBH",
                  ),
                  DropdownMenuItem(child: Text('NBH'),
                    value: "NBH",
                  ),
                  DropdownMenuItem(child: Text('APARTMENT'),
                    value: "APARTMENT",
                  ),
                  DropdownMenuItem(child: Text('GIRL HOSTEL'),
                    value: "GIRL HOSTEL",
                  ),
                  DropdownMenuItem(child: Text('OLH'),
                    value: 'OLH',
                  ),
                ],
                onChanged: (value){
...
class _SubmittedState extends State<Submitted> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
        stream: FirebaseFirestore.instance.collection("Mentee").document(email).collection("Applications").snapshots(),
                          ...
                          child: new Column(
                              children: <Widget>[
                                ...
                                teacherstatus=="Approved" && wardenstatus=="Approved"?Container(
                                  ...
                                  child: Text("REJECTED",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 18.0,color: Colors.red),)
                                )
                              ]
                          ),
                        ),
